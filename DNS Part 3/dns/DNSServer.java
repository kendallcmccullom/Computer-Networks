...
public class DNSServer {
	...
	//et the maximum packet size to be 512 bytes for DNS messages
	...
	private HashMap<Integer, DNSMessage> storing= new HashMap<Integer, DNSMessage>();
	//private ArrayList<DNSRecord> keep = new ArrayList<DNSRecord>();
	private HashMap<String, ArrayList> matching = new HashMap<String, ArrayList>();
	...
	private DatagramPacket handleQuery(DNSMessage query) {
		...
		System.out.println(query);
		storing.put(query.getID(), query);

		DatagramPacket forwarding = new DatagramPacket(query.getData(), query.getDataLength(), nextServer, nextServerPort);
	...
	private DatagramPacket handleReply(DNSMessage reply) {
		...
		System.out.println(reply);
		DNSMessage request = storing.get(reply.getID());

		cache.addCache(reply);

		storing.remove(reply.getID(), request);
		System.out.println("Fowarding Reply to " + request.getPacket().getSocketAddress());
		System.out.println(reply);
		...
		DatagramPacket finalResponse = new DatagramPacket(reply.getData(),reply.getDataLength() , request.getPacket().getSocketAddress());
	...
	private DatagramPacket handleMessage(DatagramPacket incomingPkt) {
		...
		//cache.addCache(incomingMessage);
		cache.remove();
		// handle queries
